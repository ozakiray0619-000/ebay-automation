...
| F2 | AI返信案生成（Claude API） | gas-reply / F2_AIReply.gs | ✅ キー無しでも動く「ざっくりテンプレ返信」を実装(2026-06-11)。キーワードで定型文自動選択(英/日)。`testAiReplyDraft`で確認済み。ANTHROPIC_API_KEY設定時は自動で本物AI生成に昇格 |
...
### [2026-06-10] APIキー無し対策(ローカル偽eBay)＋Sandbox実キー登録・DNSで中断

- **背景**: 「本物のeBayから実際に通知が来るか」をテストしたいが、本番API審査待ち＋Sandbox障害でAPIキー/実データが得られず詰まっていた。その対策を実施。
- **対策①: ローカル「偽eBayサーバー」一式を新規作成**（`local-test/`、Node・依存なし・ターミナルで動作）。eBayキー不要で通知パイプライン(取得→重複除去→翻訳→Discord)をE2E検証できる。eBay部分だけダミーで、Discordは本物が飛ぶ。
  - `mock-ebay-server.js`(注文REST JSON/オファーTrading XML/OAuthを本物同形式で返す) ・ `poller.js`(GAS同等ロジック＋本物Discord送信、Webhook未設定ならドライラン) ・ `scenario.json`(ダミーデータ) ・ `config.sample.json` ・ `run.ps1`(`.\run.ps1` / `-Loop 30` / `-Reset`) ・ `README.md` ・ `.gitignore`
  - ※今セッションは検証用LinuxVMがダウンで実走未。コードレビューでロジック確認済。ユーザーPCで `.\run.ps1` 実行で確定予定。
- **対策②: 本物Sandboxでの実テストに着手**
  - eBay Sandbox API Statusページ確認 → **現在アクティブな障害なし**（6/9のは一時的/局所的だった可能性）。
  - `/my/keys` で **本番Keyset `DiluNova_0011` が「無効」**＝マーケットプレイス アカウント削除通知コンプライアンス未完了。**これは本番専用ゲート。Sandboxは対象外**と判明 → テストはSandboxキーで進める方針に。
  - **GAS(gas-skeleton)スクリプトプロパティにSandbox実キー登録**: `EBAY_ENV=SANDBOX` / `EBAY_APP_ID`(SBX-***) / `EBAY_DEV_ID`(***) / `EBAY_CERT_ID`(=client secret、値はログ非記載) / 既存の `SPREADSHEET_ID` `DISCORD_WEBHOOK_URL` `EBAY_VERIFICATION_TOKEN` `OAUTH_STATE` `WEBAPP_URL` はそのまま(干渉なし)。※実キー値はGASのプロパティにのみ保持、リポジトリには残さない。
  - ⚠️ Cert IDは末尾が `...-3cff` で終わり、形式上完結の可能性ありだが**切れている懸念も残る**。`getAccessToken`/`pollOffers`で**401が出たらCert IDを最優先で取り直す**。
- **中断ポイント(次回ここから)**: Auth'n'Authトークン取得のため `accounts.sandbox.ebay.com` のサインイン/パスワード再設定に行こうとしたが、**ユーザーPCで「DNSアドレスが見つかりません」**。→ 切り分け済: **外部(Claude側)からは同ホストに到達OK＝eBay障害ではなくユーザー側ローカルDNSの問題**。対処候補: `ipconfig /flushdns`→ブラウザ再起動 / スマホテザリングで切り分け / DNSを `1.1.1.1`or`8.8.8.8` に変更 / 別ブラウザ。
  - ユーザーはSandboxテストユーザーの**パスワードを忘れており再設定したい**(reset-passwordページも同ドメイン経由のため、まずDNS解決が先決)。

**次回「続きから」= DNS解決(flushdns/テザリング/DNS変更)→ `accounts.sandbox.ebay.com` 到達→(必要ならテストユーザー作成/パスワード再設定 `developer.ebay.com/sandbox/register`)→ `/my/auth?env=sandbox` でAuth'n'Authトークン取得→GASに `EBAY_AUTH_TOKEN` 登録→ `checkProps`→ `pollOffers` 実行→ logsで `fetchActiveBestOffers ok` 確認。** すぐ通知を見たいだけなら `local-test/` のローカル偽eBay→本物Discordが最短。
