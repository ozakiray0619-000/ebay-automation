 ...
 *
 * NOTE: Trading API GetBestOffers の実レスポンスは
 *       GetBestOffersResponse → ItemBestOffersArray → ItemBestOffers → BestOfferArray → BestOffer
 *       かつ Item 情報は親の ItemBestOffers 配下にあるネスト構造。
 *       Sandbox/Production の実データで階層を確認次第、調整する。
 */
function parseBestOffersXml(xmlText) {
  ...
  // 安全なテキスト取得ヘルパ（null セーフ、空文字フォールバック）
  const textOf = (parent, childName) => {
    if (!parent) return '';
    const c = parent.getChild(childName, ns);
    return c ? c.getText() : '';
  };

  // 1件の BestOffer ノードから抽出（itemNode は親 ItemBestOffers の Item ノード、無くても可）
  const extract = (bestOfferNode, itemNode) => ({
    offerId: textOf(bestOfferNode, 'BestOfferID'),
    itemId:  textOf(itemNode, 'ItemID'),
    title:   textOf(itemNode, 'Title'),
    buyer:   textOf(bestOfferNode.getChild('Buyer', ns), 'UserID'),
    price:   textOf(bestOfferNode, 'Price'),
    status:  textOf(bestOfferNode, 'Status')
  });

  // ItemBestOffersArray > ItemBestOffers > BestOfferArray > BestOffer の階層を踏破
  const itemBestOffersArr = root.getChild('ItemBestOffersArray', ns);
  if (itemBestOffersArr) {
    const result = [];
    itemBestOffersArr.getChildren('ItemBestOffers', ns).forEach(ibo => {
      const itemNode = ibo.getChild('Item', ns);
      const bestOfferArr = ibo.getChild('BestOfferArray', ns);
      if (!bestOfferArr) return;
      bestOfferArr.getChildren('BestOffer', ns).forEach(bo => {
        result.push(extract(bo, itemNode));
      });
    });
    return result;
  }

  // フォールバック: 旧パース（直下 BestOfferArray 配下）
  return root.getChildren('BestOfferArray', ns)
    .flatMap(n => n.getChildren('BestOffer', ns))
    .map(bo => extract(bo, bo.getChild('Item', ns)));
}
